...
function c25032004.initial_effect(c)
	--Activate
	...
	e5:SetTarget(c25032004.rmtg)
	c:RegisterEffect(e5)
	--Banish GY Zaloog
	e6 = e5:Clone()
	e6:SetTarget(c25032004.rmdztg)
	c:RegisterEffect(e6)
	--to hand
	...
	e7:SetCountLimit(1)
	e7:SetCondition(c25032004.thcon)
	e7:SetTarget(c25032004.thtg)
	e7:SetOperation(c25032004.thop)
	c:RegisterEffect(e7)
end

function c25032004.rmtg(e, c)
	return c:IsReason(REASON_EFFECT) and c:GetReasonEffect():GetHandler():IsSetCard(0x1a) and c:GetOwner() ~= e:GetHandlerPlayer()
end
function c25032004.rmdztg(e, c)
	return c:IsReason(REASON_EFFECT) and c:GetReasonEffect():GetHandler():IsCode(76922029) and c:GetOwner() ~= e:GetHandlerPlayer()
end
function c25032004.thcon(e,tp,eg,ep,ev,re,r,rp)
	return ep~=tp and eg:GetFirst():IsControler(tp) and eg:GetFirst():IsSetCard(0x1a) or eg:GetFirst():IsCode(76922029)
end
function c25032004.thfilter(c)
	return c:IsSetCard(0x1a) or c:IsCode(76922029) and c:IsAbleToHand()
end
function c25032004.thtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_DECK) and chkc:IsControler(tp) and c25032004.thfilter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c25032004.thfilter,tp,LOCATION_DECK,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local sg=Duel.SelectTarget(tp,c25032004.thfilter,tp,LOCATION_DECK,0,1,1,nil)
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,sg,1,0,0)
end
function c25032004.thop(e,tp,eg,ep,ev,re,r,rp)
