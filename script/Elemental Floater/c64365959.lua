...
function s.spcon2(e,tp,eg,ep,ev,re,r,rp)
	...
	return ep==tp and c:IsControler(tp) and e:GetHandler():IsRelateToBattle()
end
function s.filter(c,e,tp,dam)
	return (c:IsRace(RACE_THUNDER) or c:IsAttribute(ATTRIBUTE_LIGHT)) and c:IsAttackBelow(dam) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
